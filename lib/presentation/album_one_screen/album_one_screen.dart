import '../album_one_screen/widgets/songs4_item_widget.dart';
import 'models/album_one_model.dart';
...
import 'provider/album_one_provider.dart';

class AlbumOneScreen extends StatefulWidget {
  const AlbumOneScreen({Key? key}) : super(key: key);

  @override
  AlbumOneScreenState createState() => AlbumOneScreenState();

  static Widget builder(BuildContext context) {
    return ChangeNotifierProvider(
        create: (context) => AlbumOneProvider(), child: AlbumOneScreen());
  }
}

class AlbumOneScreenState extends State<AlbumOneScreen> {
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget _buildSongs(BuildContext context) {
    return Align(
        ...
        child: Padding(
            ...
            child:
                Consumer<AlbumOneProvider>(builder: (context, provider, child) {
              return ListView.separated(
                  ...
                  itemCount: provider.albumOneModelObj.songs4ItemList.length,
                  itemBuilder: (context, index) {
                    Songs4ItemModel model =
                        provider.albumOneModelObj.songs4ItemList[index];
                    return Songs4ItemWidget(model);
                  });
            })));
  }
