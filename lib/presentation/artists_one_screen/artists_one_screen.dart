import '../artists_one_screen/widgets/artistsone_item_widget.dart';
import 'models/artists_one_model.dart';
...
import 'provider/artists_one_provider.dart';

class ArtistsOneScreen extends StatefulWidget {
  const ArtistsOneScreen({Key? key}) : super(key: key);

  @override
  ArtistsOneScreenState createState() => ArtistsOneScreenState();

  static Widget builder(BuildContext context) {
    return ChangeNotifierProvider(
        create: (context) => ArtistsOneProvider(), child: ArtistsOneScreen());
  }
}

// ignore_for_file: must_be_immutable
class ArtistsOneScreenState extends State<ArtistsOneScreen> {
  ...
  /// Section Widget
  Widget _buildArtistsOne(BuildContext context) {
    return Padding(
        padding: EdgeInsets.only(right: 22.h),
        child:
            Consumer<ArtistsOneProvider>(builder: (context, provider, child) {
          return ListView.separated(
              ...
              itemCount: provider.artistsOneModelObj.artistsoneItemList.length,
              itemBuilder: (context, index) {
                ArtistsoneItemModel model =
                    provider.artistsOneModelObj.artistsoneItemList[index];
                return ArtistsoneItemWidget(model);
              });
        }));
  }
